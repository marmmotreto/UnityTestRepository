...
namespace TMPro.EditorUtilities
{
    ...
    public class TMPro_EditorPanel : Editor
    {
        ...
        private Texture2D alignBaseline;
        ...
        public override void OnInspectorGUI()
        {
            ...
            {              
                ...
                selAlignGrid_B = GUI.SelectionGrid(new Rect(columnB + 23 * 4 + 10, rect.y, 23 * 4, rect.height), selAlignGrid_B, alignContent_B, 4, btn);
                ...
                EditorGUI.PrefixLabel(pos0, new GUIContent("Wrapping & Overflow"));

                pos0.x += pos0.width;
                pos0.width = (rect.width - pos0.x) / 2 + 5;               
                int wrapSelection = EditorGUI.Popup(pos0, enableWordWrapping_prop.boolValue ? 1 : 0, new string[] { "Disabled", "Enabled" });
                enableWordWrapping_prop.boolValue = wrapSelection == 1 ? true : false;
                
                pos0.x += pos0.width + 5;
                EditorGUI.PropertyField(pos0, textOverflowMode_prop, GUIContent.none);                         
                if (EditorGUI.EndChangeCheck())
                {
                    havePropertiesChanged = true;
                    //isAffectingWordWrapping_prop.boolValue = true;

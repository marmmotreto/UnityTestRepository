...
namespace TMPro.EditorUtilities
{
    ...
    public class TMPro_FontAssetCreatorWindow : EditorWindow
    {
        ...
        private string[] FontResolutionLabels = { "16","32", "64", "128", "256", "512", "1024", "2048", "4096" };
        private int[] FontAtlasResolutions = { 16, 32, 64, 128, 256, 512, 1024, 2048, 4096 };
            ...
            if (GUILayout.Button("Generate Font Atlas", GUILayout.Width(290)) && characterSequence.Length != 0 && GUI.enabled)
            {
                if (font_TTF != null)
                {
                    ...
                    if (error_Code == 0)
                    {
                        ...
                        {
                            isRenderingDone = false;
                            error_Code = TMPro_FontPlugin.Render_Characters(m_texture_buffer, font_atlas_width, font_atlas_height, padding, character_Set, m_character_Count, font_style, font_style_mod, autoSizing, font_renderMode,(int)m_fontPackingSelection, ref m_font_faceInfo, m_font_glyphInfo);
        ...
        void Save_Normal_FontAsset(string filePath)
        {
            ...
            if (index == -1) index = filePath.LastIndexOf("assets");

            string relativeAssetPath = filePath.Substring(index);
        ...
        void Save_SDF_FontAsset(string filePath)
        {
            ...
            if (font_asset == null)
            {
                ...
                tmp_material.SetFloat(ShaderUtilities.ID_TextureHeight, m_font_Atlas.width);
        ...
        void DrawPreview()
        {
            ...
            GUILayout.BeginVertical(textureAreaBox);

            Rect pixelRect = GUILayoutUtility.GetRect(m_previewWindow_Size.x, m_previewWindow_Size.y, SectionLabel);

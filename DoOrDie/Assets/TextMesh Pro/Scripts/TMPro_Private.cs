...
{  
    ...
    public partial class TextMeshPro : MonoBehaviour
    {
        ...
        private TextAlignmentOptions m_lineJustification = TextAlignmentOptions.Left;
        ...
        //private WordWrapState m_SavedLastCharState = new WordWrapState();
        //private WordWrapState[] m_SavedCharacterStates = new WordWrapState[5];
       ...
       void OnDisable()
        {
            //Debug.Log("TextMeshPro OnDisable() called. Renderer.MeshFilter ID " + m_renderer.GetComponent<MeshFilter>().sharedMesh.GetInstanceID() + "  Mesh ID " + m_mesh.GetInstanceID() + "  MeshFilter ID " + m_meshFilter.GetInstanceID()); //has been called. HavePropertiesChanged = " + havePropertiesChanged); // has been called on Object ID:" + gameObject.GetInstanceID());      
            //if (m_meshFilter.sharedMesh != null)
        ...
        void ON_DRAG_AND_DROP_MATERIAL(Material currentMaterial, Material newMaterial)
        {
            ...
            if (currentMaterial.GetInstanceID() == m_sharedMaterial.GetInstanceID())
            {
        ...
        {                      
            ...
            if (m_transform.hasChanged)
            {
                ...
                if (m_textContainer != null && m_textContainer.hasChanged)
                {
                    ...
                    //Debug.Log("Text Container has changed. isTextTruncated is " + m_isTextTruncated);
        ...
        {
            //Debug.Log("TextMeshPro GenerateTextMesh() called. Renderer.MeshFilter ID " + m_renderer.GetComponent<MeshFilter>().GetInstanceID() + "  Mesh ID " + m_mesh.GetInstanceID() + "  MeshFilter ID " + m_meshFilter.GetInstanceID()); //has been called. HavePropertiesChanged = " + havePropertiesChanged); // has been called on Object ID:" + gameObject.GetInstanceID());      
            ...
            float marginWidth = m_textContainer.rect.width - margins.z - margins.x;
            float marginHeight = m_textContainer.rect.height - margins.y - margins.w;
            //Debug.Log("Margins Width: " + marginWidth + "  Height: " + marginHeight);

            // Initialize struct to track states of word wrapping
            m_SavedWordWrapState = new WordWrapState();
            //m_SavedLastCharState = new WordWrapState();
            //m_SavedLineState = new LineWrapState();
            int wrappingIndex = 0;
            ...
            for (int i = 0; m_char_buffer[i] != 0; i++)
            {
                ...
                //m_textInfo.characterInfo[character_Count].charNumber = (short)character_Count;
                ...
                if (charCode != 32 && charCode != 9 && charCode != 10 && charCode != 13)
                {
                   ...
                   // Debug.Log("Line # " + m_lineNumber + " BaseScale: " + baseScale + "  MaxFontScale: " + m_maxFontScale);     
                    ...
                    if (m_textInfo.characterInfo[m_characterCount].topRight.x - (m_padding - m_alignmentPadding.z) * m_fontScale > marginWidth && !m_textContainer.isDefaultWidth)
                    {
                        //Debug.Log(m_textInfo.characterInfo[m_characterCount].topRight.x + "   Padding: " + (m_padding * m_fontScale) + "  Alignment Padding: " + (m_alignmentPadding.z * m_fontScale));
                        // Word Wrapping
                        ...
                        if (enableWordWrapping)
                        {
                            ...
                            {                         
                                ...
                                AdjustLineOffset(firstCharacterOfLine, m_characterCount, offsetDelta - lineOffsetDelta);
                            ...
                            m_preferredWidth += m_xAdvance; // m_textInfo.characterInfo[m_textInfo.lineInfo[m_lineNumber].lastCharacterIndex - 1].topRight.x + (17.21875f * m_maxFontScale);
                        ...
                        switch (m_overflowMode)
                        {
                            ...
                            case TextOverflowModes.Ellipsis:
                                ...
                                /* // Better performance but not sure the extra complexity is warranted.
                                if (i < 1) break;

                                m_char_buffer[i - 1] = 8230;
                                m_char_buffer[i] = (char)0;
                                    
                                if (m_characterCount < 2)
                                {
                                    i = RestoreWordWrappingState(ref m_SavedCharacterStates[(m_characterCount - 1) % 5]);
                                    m_isTextTruncated = true;
                                    continue;                                  
                                }

                                i = RestoreWordWrappingState(ref m_SavedCharacterStates[(m_characterCount - 2) % 5]);
                                //Debug.Log("Restoring at Index " + i);
                                //wrappingIndex = i;
                                m_isTextTruncated = true;
                                continue;
                                */
                            ...
                            case TextOverflowModes.Truncate:
                                ...
                                //Debug.Log("Line Truncated at Character [" + (char)charCode + "] at Index: " + character_Count);

                                //truncateIndex = character_Count;
                                m_textInfo.characterInfo[m_characterCount].isVisible = false;
                ...
                {                                              
                    //Debug.Log("Line # " + m_lineNumber + "  Previous Character is [" + m_textInfo.characterInfo[lastCharacterOfLine].character + "].");
                ...
                if (m_maxAscender - m_maxDescender + (m_alignmentPadding.w * 2 * m_fontScale) > marginHeight && !m_textContainer.isDefaultHeight)
                {
            ...
            if ((!m_textContainer.isDefaultWidth || !m_textContainer.isDefaultHeight) && !m_isCharacterWrappingEnabled && (m_enableAutoSizing && fontSizeDelta > 0.25f && m_fontSize < m_fontSizeMax))
            {
            ...
            for (int i = 0; i < m_characterCount; i++)
            {
                ...
                switch (lineAlignment)
                {
                    ...
                    case TextAlignmentOptions.BottomJustified:
                        ...
                        {   // All lines are justified accept the last one.
                            float gap = (corners[3].x - margins.z) - (corners[0].x + margins.x) - (lineInfo.lineExtents.max.x);
                ...
                else
                {
                    if (beginUnderline == true)
                    {
                        ...
                        if (i != m_characterCount - 1 && (m_textInfo.characterInfo[i].character == 32 || m_textInfo.characterInfo[i].character == 10)) // True if not last character and either a space or linefeed                      
            ...
            m_mesh.bounds = new Bounds(new Vector3((m_meshExtents.max.x + m_meshExtents.min.x) / 2, (m_meshExtents.max.y + m_meshExtents.min.y) / 2, 0) + m_anchorOffset, new Vector3(m_meshExtents.max.x - m_meshExtents.min.x, m_meshExtents.max.y - m_meshExtents.min.y, 0));              
            ...
            if ((m_textContainer.isDefaultWidth || m_textContainer.isDefaultHeight) && m_textContainer.isAutoFitting)
            {
                ...
                {
                    m_textContainer.height = m_maxAscender + margins.y - m_meshExtents.min.y + margins.w + m_alignmentPadding.y * m_fontScale * 2;
            ...
            /*
            for (int i = 0; i < m_textInfo.lineCount; i++)
            {              
                //Debug.Log("Line: " + (i + 1) + "  # Char: " + m_textInfo.lineInfo[i].characterCount
                //                             + "  Word Count: " + m_textInfo.lineInfo[i].wordCount
                //                             + "  Space: " + m_textInfo.lineInfo[i].spaceCount
        ...
        void SaveWordWrappingState(ref WordWrapState state, int index)
        {
            ...
            //state.characterInfo = m_textInfo.characterInfo;
        ...
        int RestoreWordWrappingState(ref WordWrapState state)
        {
            ...
            //m_textInfo.characterInfo = state.characterInfo;
        ...
        //    m_SavedLineState.lineInfo = m_textInfo.lineInfo[m_lineNumber];
        ...
        //    m_textInfo.lineInfo[m_lineNumber] = m_SavedLineState.lineInfo;
